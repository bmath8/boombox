-- Discovery Features Schema

-- 1. User Affinities (Cache for faster matching)
CREATE TABLE IF NOT EXISTS public.user_affinities (
    user_id UUID REFERENCES public.users(user_id) ON DELETE CASCADE,
    genre VARCHAR(100),
    artist VARCHAR(100),
    affinity_score INTEGER DEFAULT 1, -- 1-100
...
-- 2. Discovery Matches (Potential friends)
CREATE TABLE IF NOT EXISTS public.discovery_matches (
    ...
    status VARCHAR(20) DEFAULT 'new', -- new, viewed, dismissed, connected
    created_at TIMESTAMP DEFAULT NOW(),
    UNIQUE(user_id, matched_user_id)
);

-- 3. Function to calculate affinity based on listening history
